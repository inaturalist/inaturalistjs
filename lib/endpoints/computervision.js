...
const computervision = class computervision {
  ...
  static language_search( params, opts = { } ) { // eslint-disable-line camelcase
    const options = { ...opts };
    options.useAuth = true;
    return iNaturalistAPI.get( "computervision/language_search", params, options )
      .then( response => {
        response.results = response.results.map( r => (
          { ...r, observation: new Observation( r.observation ) }
        ) );
        return response;
      } );
  }
